...
def train_one_epoch(
    ...
    """
    Suporta dois formatos de batch:
      - só imagem: (imgs, targets)
      - fusão:      (imgs, tabs, targets)
    """
    model.train()
    ...
    for batch in loader:
        if len(batch) == 3:
            ...
            tabs = tabs.to(device, non_blocking=False)
            targets = targets.to(device)
            # mixup: evite usar com fusão (targets soft + tabular fixo = mismatch)
        ...
        if targets.ndim == 2:  # mixup ativo → targets soft
...
def evaluate(model, loader: DataLoader, device):
    """
    Avaliação padrão. Suporta (imgs, tabs, targets) e (imgs, targets).
    """
    model.eval()
    ...
    for batch in loader:
        if len(batch) == 3:
            imgs, tabs, targets = batch
            imgs = imgs.to(device); tabs = tabs.to(device); targets = targets.to(device)
            with autocast_ctx(device):
                outputs = model(imgs, tabs)
...
def predict_all_logits(model, loader, device):
    """
    Retorna y_true e logits do modelo atual, suportando batches com ou sem tabular.
    """
    model.eval()
    ys, logits = [], []
    for batch in loader:
        if len(batch) == 3:
            ...
            with autocast_ctx(device):
                logit = model(imgs, tabs)  # [N, C]
        else:
            imgs, y = batch
            imgs = imgs.to(device)
            with autocast_ctx(device):
                logit = model(imgs)        # [N, C]
...
def compute_probs_and_metrics(logits, y_true, pos_idx):
    ...
    # Métricas na orientação invertida
...
def main():
    ...
    # -------- NOVO: fusão imagem+tabular --------
    p.add_argument("--fusion_tab_csv_glob", type=str, default="",
                   help='Ativa fusão (imagem+tabular). Ex.: "data/csv/*_location_fixed.csv"')
    ...
    if args.cv_splits and args.cv_splits > 1:
        # CV estratificado (apenas imagem). Se quiser CV com fusão,
        # use um wrapper equivalente a build_kfold_dataloaders_fusion.
        ...
        if fusion_active:
            print("[WARN] --cv_splits ativo, mas a rota de CV+fusão não está implementada aqui. "
                  "Use treino simples (sem CV) ou crie um build_kfold_dataloaders_fusion.")
    else:
        if fusion_active:
    ...
    # Model (agora com tab_dim quando fusão ativa)
    ...
    # Loss / Mixup
    if fusion_active:
        # Evite mixup/cutmix com fusão (tabular não é mixado)
        mixup_fn = None
        loss_fn = nn.CrossEntropyLoss(label_smoothing=args.smoothing if args.smoothing > 0 else 0.0)
    else:
    ...
    if ckpt_path and ckpt_path.exists():
        ...
        best_monitor = ckpt.get("best_acc", -1.0)  # retrocompat
        ...
        try:
            ...
            for epoch in range(start_epoch, args.epochs):
                ...
                torch.save({
                    ...
                    "img_size": args.img_size,
                    # dica: se quiser, salve também tab_dim para sanidade
